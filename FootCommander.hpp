...
public:
    ...
    static void allSoldiersAttack(std::vector<std::vector<Soldier*>> &board,int player) {
        for (int i = 0; i < board.size(); ++i) {
            for (int j = 0; j < board.size(); ++j) {
                if(board[i][j]!=nullptr &&
                board[i][j]->player==player &&
                (board[i][j]->SoldierType==1 || board[i][j]->SoldierType==0)){
    ...
    void attack(std::vector<std::vector<Soldier*>> &board, std::pair<int,int> pos) override{
        allSoldiersAttack(board,this->player);
        bool found=false; int rowFrom=pos.first-1,rowTo=pos.second+1,colFrom=pos.first-1,colTo=pos.second+1;
        while(!found){
            ...
            for (int i=rowFrom; i<rowTo; ++i){
                for (int j=colFrom; j<colTo; ++j){
                    ...
                    found = true; std::cout<<"Enemy spotted at"<<enemy<<" R:"<<i<<"L:"<<j<<"\tattacking!"<<std::endl;
                    enemy->healthPoints -=this->AttcakDamage;
                    if(enemy->healthPoints <= 0){ board[i][j]=nullptr; std::cout <<"Enemy Down!"<<std::endl; }
                }
            }
            rowFrom-=1; rowTo+=1;
            colFrom-=1; colTo+=1;
